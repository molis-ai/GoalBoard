const storageKey = 'goalboard-timeline-design-v1';
...
function demoConfig() {
  ...
  if (state.scenario === 'empty') return { ...base, title: '让第一次使用 GoalBoard 更顺畅', outcome: '先把这个想法留下。你已经表达过的意图，会成为接下来工作的起点。', status: '尚未开始', actor: '还没有执行会话', note: '可以先梳理现状；当前尚未约定完成要求', next: '结合当前使用路径，整理希望改善的结果和明显差距。', owner: '待分配', helper: '无需先填写完整计划才能保存目标。' };
  if (state.scenario === 'parent') return { ...base, title: '让 GoalBoard 可以交给第一批用户试用', outcome: '安装、第一次协作、遇到问题后的恢复，都能连成一条真实可用的体验。', status: '进行中', actor: '首次协作子目标正在处理', note: '下一步：补齐首次协作，再走一次完整试用', next: '接上首次协作结果，再从安装开始验证完整试用。', owner: 'Codex · 首次协作；一骏 · 试用反馈', helper: '子目标都完成后，仍要核对整条试用体验。' };
  return base;
}
...
function sourceHtml(type) {
  ...
  const sources = {
    ...
    parent: ['子目标成果汇总', 'GoalBoard（示例）', '9 月 9 日 14:32', '安装路径已完成，首次协作待接续，整体验证未运行。', '聚合保留来源；子目标完成不能自动证明整体验证通过。'],
...
function handleAction(action, opener) {
  ...
  if (action === 'children') openReader('整条试用体验', `<p class="reader-lead">让 GoalBoard 可以交给第一批用户试用</p><h3>安装与启动</h3><p>已有结果：新机器安装成功，能打开工作区。供首次协作使用。</p><h3>第一次目标协作</h3><p>${state.scenario === 'complete' ? '项目创建、第一个结果与重启接续均已验证，当前目标已完成。' : '项目创建和第一个结果已验证；重启接续仍在处理。'}这就是当前原型演示的目标。</p><h3>完整试用</h3><p>${state.scenario === 'complete' ? '现在可以使用安装与首次协作的结果，从头走一次完整试用；这项整体验证尚未运行。' : '使用安装和首次协作的结果，从头走完一次。尚未验证；它需要前两项的真实产物。'}</p><h3>责任与承接</h3><p>Codex 汇总路径与结果，一骏提供试用反馈。子目标的过程留在各自时间流；这里只呈现整体验收需要的成果和差距。</p><h3>没有丢掉的要求</h3><p>目标不因子项数量归零自动完成。整合体验需要对应的实际结果。</p>`, opener);
}
