...
## persistence
pass. Root `PRODUCT.md` exists (old five-tab copy; packet forbids using it as visual authority). Not a generated-comp build: no `hero-repro.png` owed. Direction is `specs/goal-timeline-redesign/DESIGN.md` (approved v3 Operate/Read); 04 spec overrides prototype simulation. No comp-round mocks requiring an approval record. Detector `04-design-detector.json` is `[]` (static only).
