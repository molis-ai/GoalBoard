...
## 目标与范围
用户确认在目录归拢后验收新构建的安装、首次使用、目标闭环与重启恢复，达到内部完整后再处理日用更新。基准提交 7b00098。使用临时 GoalBoard Home、临时 npm consumer 和独立 Web 端口；现用项目、Runtime 配置、4173 服务保持不动。不得公开发布。已有 desktop 图片删除保持未提交。
...
3. 桌面 App 的健康检查和 URL 固定 4173；临时 GOALBOARD_HOME 不隔离端口。先明确可行启动方法，不能将连接旧服务算新构建服务验收。
