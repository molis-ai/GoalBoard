...
import { SERVICE_OWNER, SERVICE_LABEL, GoalBoardWebServiceError, type GoalBoardWebServiceDetection, type WebServiceReceipt } from "./web-service-contract.js";
...
export class WebServiceOperations {
  ...
  private detect(): Promise<GoalBoardWebServiceDetection> { return detectWebService(this.environment); }
  async install(plist: string): Promise<GoalBoardWebServiceDetection> {
  ...
  async startPreservingState(previouslyRunning: boolean): Promise<GoalBoardWebServiceDetection> {
    await this.process.assertPortAvailable();
    try {
  ...
  async restart(): Promise<GoalBoardWebServiceDetection> {
