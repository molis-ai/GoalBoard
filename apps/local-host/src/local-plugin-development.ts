...
import { createGoalBoardLocalHost, goalBoardHostProjectReference } from "./project-host.js";
...
export async function runLocalPluginDevelopment(input: {
  ...
  const markerName = ".goalboard-plugin-development.json";
  const marker = { kind: "goalboard-plugin-development", schema_version: 1 };
  ...
  try {
    return await host.client(goalBoardHostProjectReference({ databasePath: join(directory, "development.db"), boardId }))
