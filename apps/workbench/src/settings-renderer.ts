...
import type { RuntimeIntegrationDetection } from "@adeptify/goalboard-contracts/platform/app-host";
import type { GoalBoardSettingsView } from "./settings-view.js";
...
function renderRuntimeSettings(view: GoalBoardSettingsView): string {
  ...
  return `<section class="settings-document" aria-labelledby="settings-title">
    ...
    <div class="settings-body"><div class="settings-record-list">${rows || `<div class="settings-empty"><h2>${L("没有可探测的 Runtime")}</h2><p>${L("GoalBoard 本体仍可使用；稍后安装 Runtime 后再回来检查。")}</p></div>`}</div>
...
function renderProjectSettings(view: GoalBoardSettingsView): string {
  ...
  return `<section class="settings-document" aria-labelledby="settings-title">
    ...
    <div class="settings-body"><section class="settings-action-section" aria-labelledby="create-project-title"><div><h2 id="create-project-title">${L("创建项目")}</h2><p>${L("创建一个空的 GoalBoard 项目，然后直接打开它的 Goal Tree。")}</p></div><form class="inline-settings-form" data-project-create><label>${L("项目名称")}<input name="display_name" required maxlength="160" placeholder="${L("例如：新产品发布")}"></label><label class="inline-confirm"><input type="checkbox" name="user_confirmed"><span>${L("确认创建这个项目")}</span></label><p class="settings-form-error" role="alert" hidden></p><button type="submit">${L("创建并打开")}</button></form></section>
...
function renderDiagnosticsSettings(view: GoalBoardSettingsView): string {
  ...
  return `<section class="settings-document" aria-labelledby="settings-title">
    ...
    <p class="settings-footnote">${L("如果本体不完整，请在终端重新运行 ")}<code>goalboard install</code>${L("。常驻服务操作会先展示预览并要求确认；不会在后台使用 nohup。")}</p></div>
  </section>`;
}
...
function renderGoalBoardSettings(view: GoalBoardSettingsView, controlToken = "", desktopShell = false): string {
...
<body class="settings-page" data-settings-section="${view.section}" data-desktop-shell="true"${desktopShell ? ' data-native-desktop="true"' : ""}>
  ...
  return renderGoalBoardSettings;
}
