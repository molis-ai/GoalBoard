...
test("Host migration 30 rolls back Goals and Execution together, then preserves original revisions across reopen", () => {
  ...
  try {
    ...
    const failure = spawnSync(process.execPath, ["--import", "tsx", "--input-type=module", "-e",
      `import {LocalProjectDatabase} from '@adeptify/goalboard-app-local-host'; try {new LocalProjectDatabase(process.argv[1]); process.exitCode=1} catch(e) {if(!String(e).includes('migration30 failure')) throw e}`, path], { encoding: "utf8" });
