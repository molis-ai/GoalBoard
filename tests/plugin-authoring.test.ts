...
function manifest() {
  return {
    ...
    plugin_id: "io.goalboard.example.notes", version: "1.0.0", name: "Notes", kind: "integration",
...
test("real Plugin CLI validates in a clean directory without executing entrypoint or modifying input", () => {
  const directory = mkdtempSync(join(tmpdir(), "goalboard-plugin-cli-"));
  try {
    ...
    const cli = fileURLToPath(new URL("../tooling/plugin-cli/bin/goalboard-plugin.mjs", import.meta.url));
    ...
    assert.deepEqual(JSON.parse(valid.stdout), { valid: true, plugin_id: "io.goalboard.example.notes", version: "1.0.0" });
