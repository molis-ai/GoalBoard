...
for (const owned of [true, false]) {
  test(`uninstall preview ${owned ? "classifies legacy user data without migrating it" : "refuses a catalog owned by another application"}`, async () => {
    const directory = await mkdtemp(join(tmpdir(), "goalboard-uninstall-catalog-"));
    try {
      ...
      try {
        ...
        `);
        db.prepare("INSERT INTO catalog_meta VALUES ('owner', ?)").run(owned ? "goalboard-project-catalog-v1" : "other-app");
      ...
      const service = createLocalUninstallService({
        ...
        webServiceManager: new GoalBoardWebServiceManager({ homeDirectory: home, userHomeDirectory: directory, platform: "linux" }),
      });
