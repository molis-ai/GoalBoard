...
test("Runtime state tools record progress and close without applying user identity; management decides", async () => {
  ...
  try {
    ...
    const created = JSON.parse(await runtime.callTool("goalboard_v1_goal_intent_create", { title: "内部试用故事", outcome: "玩家能走完开场", idempotency_key: "intent-state",
    }));
    const goal_id = created.goal.goal_id as string;
    await runtime.callTool("goalboard_v1_event_configure", { goal_id, expected_version: 0, idempotency_key: "cfg-state",
      types: [localType()],
    });
    const configured = JSON.parse(await runtime.callTool("goalboard_v1_goal_state", { goal_id })) as GoalEventStateView;
    await runtime.callTool("goalboard_v1_event_agree", { goal_id, idempotency_key: "agree-state",
    ...
    const reported = JSON.parse(await runtime.callTool("goalboard_v1_event_report", { goal_id, idempotency_key: "report-state",
      events: [{
    ...
    const progress = JSON.parse(await runtime.callTool("goalboard_v1_event_progress", { goal_id, idempotency_key: "progress-1",
    ...
    const asked = JSON.parse(await runtime.callTool("goalboard_v1_event_decision_request", { goal_id, idempotency_key: "ask-1",
      question: "开场是否可以内部试用？",
      options: [
    ...
    await assert.rejects(
      () => runtime!.callTool("goalboard_v1_event_decide", { goal_id, idempotency_key: "runtime-decide", conclusion: "伪造批准",
      }),
    ...
    await assert.rejects(
      () => runtime!.callTool("goalboard_v1_event_progress", { goal_id, idempotency_key: "fake-user", actor_kind: "user",
    ...
    const ready = JSON.parse(await runtime.callTool("goalboard_v1_goal_state", { goal_id })) as GoalEventStateView;
    const closed = JSON.parse(await runtime.callTool("goalboard_v1_event_close", { goal_id, idempotency_key: "close-1", kind: "complete",
    ...
    await runtime.close();
    runtime = new GoalBoardServer("runtime", connection, runtimeHost, host);
    const reopened = JSON.parse(await runtime.callTool("goalboard_v1_goal_state", { goal_id })) as GoalEventStateView;
    ...
    const progressEvent = JSON.parse(await runtime.callTool("goalboard_v1_event_read", { goal_id, event_id: progress.event_id,
    }));
    assert.equal(progressEvent.payload.operation, "progress_summary");
    assert.equal(progressEvent.payload.summary, "开场可玩");

    const beforeClose = JSON.parse(await runtime.callTool("goalboard_v1_event_list", { goal_id, limit: 100 }));
    await assert.rejects(
      () => runtime!.callTool("goalboard_v1_event_close", { goal_id, idempotency_key: "close-invalid", kind: "not-a-valid-kind",
    ...
    const afterInvalid = JSON.parse(await runtime.callTool("goalboard_v1_event_list", { goal_id, limit: 100 }));
...
test("Runtime agree unknown field and omitted close version are rejected with no write", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goalboard-mcp-goal-events-unknown-"));
  const homeDirectory = join(directory, "home");
  const catalog = await openGoalBoardProjectCatalog({ homeDirectory });
  const host = createGoalBoardLocalHost();
  let runtime: GoalBoardServer | undefined;
  try {
    ...
    runtime = new GoalBoardServer("runtime", {
      databasePath: project.database_path,
      boardId: project.board_id,
      projectId: project.project_id,
      webBaseUrl: "http://127.0.0.1:4173",
    ...
    const board_id = project.board_id;
    const created = JSON.parse(await runtime.callTool("goalboard_v1_goal_intent_create", { title: "未知字段", outcome: "原结果", idempotency_key: "intent-unknown",
    }));
    const goal_id = created.goal.goal_id as string;
    await runtime.callTool("goalboard_v1_event_configure", { goal_id, expected_version: 0, idempotency_key: "cfg-unknown", types: [localType()],
    });
    const state = JSON.parse(await runtime.callTool("goalboard_v1_goal_state", { goal_id })) as GoalEventStateView;
    await assert.rejects(
      () => runtime!.callTool("goalboard_v1_event_agree", { goal_id, idempotency_key: "agree-unknown",
        expected_config_version: state.config.version,
        expected_agreement_version: state.agreement.version,
        outcome: "新结果",
        independently_verified: true,
      }),
      (error: unknown) => error instanceof GoalBoardV1Error && error.code === "mcp.unexpected_field",
    );
    await assert.rejects(
      () => runtime!.callTool("goalboard_v1_event_close", { goal_id, idempotency_key: "close-omit", kind: "complete",
    ...
    const after = JSON.parse(await runtime.callTool("goalboard_v1_goal_state", { goal_id })) as GoalEventStateView;
...
test("MCP agreement_change request keeps request-time commitment; later related change rejects approval", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goalboard-mcp-goal-events-stale-"));
  const homeDirectory = join(directory, "home");
  const catalog = await openGoalBoardProjectCatalog({ homeDirectory });
  const host = createGoalBoardLocalHost();
  let runtime: GoalBoardServer | undefined;
  ...
  try {
    ...
    runtime = new GoalBoardServer("runtime", connection, {
      homeDirectory,
      runtimeContext: { runtime_id: "codex", stable_work_context_id: "thread-stale", host_declares_stable: true },
    }, host);
    management = new GoalBoardServer("management", connection, null, host);
    const board_id = project.board_id;
    const created = JSON.parse(await runtime.callTool("goalboard_v1_goal_intent_create", { title: "精确授权", outcome: "用户能完成真实购买", idempotency_key: "intent-stale-mcp",
    }));
    const goal_id = created.goal.goal_id as string;
    await runtime.callTool("goalboard_v1_event_configure", { goal_id, expected_version: 0, idempotency_key: "cfg-stale-mcp", types: [localType()],
    });
    const configured = JSON.parse(await runtime.callTool("goalboard_v1_goal_state", { goal_id })) as GoalEventStateView;
    await runtime.callTool("goalboard_v1_event_agree", { goal_id, idempotency_key: "agree-stale-mcp",
      expected_config_version: configured.config.version,
      expected_agreement_version: configured.agreement.version,
      new_requirements: [{ requirement_id: "r-five", statement: "真实购买" }],
    });
    const asked = JSON.parse(await runtime.callTool("goalboard_v1_event_decision_request", { goal_id, idempotency_key: "ask-retire-mcp",
    ...
    const beforeRevise = JSON.parse(await runtime.callTool("goalboard_v1_goal_state", { goal_id })) as GoalEventStateView;
    await management.callTool("goalboard_v1_event_agree", {
      database_path: project.database_path,
    ...
    const beforeApprove = JSON.parse(await runtime.callTool("goalboard_v1_goal_state", { goal_id })) as GoalEventStateView;
    await assert.rejects(
      () => management!.callTool("goalboard_v1_event_decide", {
        database_path: project.database_path,
    ...
    const afterApprove = JSON.parse(await runtime.callTool("goalboard_v1_goal_state", { goal_id })) as GoalEventStateView;
