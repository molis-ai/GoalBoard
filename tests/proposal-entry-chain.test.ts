...
import type { GoalTreeProposalDecisionResult, GoalTreeProposalListResult, GoalTreeApplicationApi } from "@adeptify/goalboard-plugin-goals";
...
test("CLI and MCP share current Goal/Relation proposal decisions across Host restart", async () => {
  ...
  const runtime = new GoalBoardServer("runtime", { databasePath, boardId, webBaseUrl: "http://127.0.0.1:4173" }, runtimeHost, host);
  ...
  try {
    ...
    const note = JSON.parse(await runtime.callTool("goalboard_v1_event_note", {
      goal_id: "draft", body: "用户确认以后才能创建子目标", idempotency_key: "source-note",
    }));
    assert.equal(note.recorded, true);
    const replayNote = JSON.parse(await runtime.callTool("goalboard_v1_event_note", {
      goal_id: "draft", body: "用户确认以后才能创建子目标", idempotency_key: "source-note",
    }));
    ...
    assert.deepEqual(JSON.parse(await runtime.callTool("goalboard_v1_goal_tree_check", {
    ...
    await assert.rejects(
      () => runtime.callTool("goalboard_v1_goal_tree_decide", {
        proposal_id: proposed.proposal.proposal_id,
        user_confirmed: true,
      ...
      (error: unknown) => error instanceof GoalBoardV1Error || /确认|user_impersonation|Web|权限|拒绝|unsupported/i.test(String(error)),
    );
