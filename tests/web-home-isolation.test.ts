...
import { createFileSecretStore, resetSecretStoreCache, runWithGoalBoardHome } from "@adeptify/goalboard-storage";
...
test("Web resolves environment Home once for Catalog, Sessions, control token and Feed", async (t) => {
  const root = mkdtempSync(join(tmpdir(), "goalboard-web-env-home-"));
  const envHome = join(root, "configured");
  const defaultHome = join(root, "default-user");
  const originalEnv = { ...process.env };
  t.mock.method(os, "homedir", () => defaultHome);
  Object.assign(process.env, { GOALBOARD_HOME: envHome, GOALBOARD_SECRET_BACKEND: "file", NODE_ENV: "test", RELAY_DB_PATH: join(root, "missing-relay.sqlite") });
  delete process.env.GOALBOARD_GITHUB_TOKEN;
  delete process.env.GOALBOARD_GMAIL_ACCESS_TOKEN;
  let server: Server | undefined;
  try {
    const catalog = await openGoalBoardProjectCatalog({ homeDirectory: envHome });
    const { project } = await catalog.ensureDemoProject({ actor_id: "env-home-test", user_confirmed: true });
    catalog.close();
    const start = async () => {
      server = createGoalBoardWebServer();
    ...
    response = await fetch(origin + feedPath + "/connectors/github/token", {
      method: "POST", headers: { origin, "content-type": "application/json", "x-goalboard-control-token": token, "x-goalboard-idempotency-key": "env-home-synthetic-bind" },
...
test("explicit Web homes isolate connector writes and survive service recreation", async () => {
  const root = mkdtempSync(join(tmpdir(), "goalboard-web-homes-"));
  const originalEnv = { ...process.env };
  const servers: Server[] = [];
  Object.assign(process.env, {
    GOALBOARD_HOME: join(root, "default"), GOALBOARD_SECRET_BACKEND: "file",
  ...
  async function start(i: number) {
    const server = createGoalBoardWebServer({ homeDirectory: homes[i], databasePath: databases[i], boardId: DEMO_BOARD_ID, controlToken });
    servers.push(server);
    await new Promise<void>((resolve) => server.listen(0, "127.0.0.1", resolve));
    const address = server.address();
    assert.ok(address && typeof address === "object");
  ...
  try {
    ...
    await Promise.all(urls.map(async (url, i) => {
      const response = await fetch(`${url}/api/feed/connectors/github/token`, {
        method: "POST", headers: { origin: url, "content-type": "application/json", "x-goalboard-control-token": controlToken, "x-goalboard-idempotency-key": `isolated-write-${i}` },
...
test("async home scopes and retained stores keep secrets and body keys in their creation home", async () => {
  const root = mkdtempSync(join(tmpdir(), "goalboard-storage-homes-"));
  ...
  try {
    const pending = runWithGoalBoardHome(join(root, "a"), async () => {
    ...
    const b = runWithGoalBoardHome(join(root, "b"), () => {
      const secrets = createFileSecretStore();
    ...
    runWithGoalBoardHome(join(root, "b"), () => a.secrets.put(authRef, "a-updated"));
    ...
    resetSecretStoreCache();
    assert.equal(runWithGoalBoardHome(join(root, "a"), () => createFileSecretStore().get(authRef)), "a-updated");
    assert.equal(runWithGoalBoardHome(join(root, "b"), () => createFileSecretStore().get(authRef)), "b-only");
  } finally {
    resetSecretStoreCache();
    if (oldBackend === undefined) delete process.env.GOALBOARD_SECRET_BACKEND;
