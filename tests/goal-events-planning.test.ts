...
import { BUILTIN_PLANNING_METHOD_PACKS } from "@adeptify/goalboard-module-goals";
import { GoalBoardServer } from "../apps/desktop/launchers/mcp/server.js";
import type { GoalEventConfigView, PlanningMethodPackInput } from "@adeptify/goalboard-contracts/modules/goals";
...
test("engineering planning drives configure/report; no template is not auto-adopted; method upgrades do not rewrite adopted Goals", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goalboard-goal-events-planning-"));
...
test("configure idempotency uses the original request before resolving upgraded templates", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goalboard-goal-events-idempotency-"));
  const homeDirectory = join(directory, "home");
  const catalog = await openGoalBoardProjectCatalog({ homeDirectory });
  let mcp: GoalBoardServer | undefined;
  try {
    ...
    const explicitReplay = JSON.parse(await mcp.callTool("goalboard_v1_event_configure", explicitInput));
...
test("equivalent planning packs merge with provenance; conflicting packs fail atomically", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goalboard-goal-events-merge-"));
  const homeDirectory = join(directory, "home");
  const catalog = await openGoalBoardProjectCatalog({ homeDirectory });
  let mcp: GoalBoardServer | undefined;
  try {
