...
Available's `action_projections` is the public action entrypoint. Read the chosen Contract, then send the returned `action_id` and `action_token` to `select_goal`. After lifecycle writes, consume `transition.projection` directly. A completed report plus required Evidence releases the executor automatically; the final Review/Risk gate completes the Goal automatically. Do not append `complete → release` to the normal workflow. Follow returned completion-blocker reasons without repeating finished execution.
