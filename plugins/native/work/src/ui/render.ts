...
export function renderWorkSessionSurface(surface: WorkUiSurface, model: WorkUiModel): string {
  ...
  switch (surface) {
    case "root": return `<button class="desktop-module-item" type="button" data-directory-open="sessions" data-work-surface-open="sessions">${icon("terminal")}<span><strong>Sessions</strong><small>执行内容、运行位置与续跑</small></span>${icon("chevron-right")}</button>`;
